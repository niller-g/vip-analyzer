...
//! them into tests.
#![expect(clippy::disallowed_types, clippy::print_stdout)]
...
pub(crate) fn generate(check: bool) {
    ...
    let ok_tests = tests.ok.values().sorted_by(|a, b| a.name.cmp(&b.name)).map(|test| {
        ...
        let test_file = format!("test_data/parser/inline/ok/{test_name}.pro");
        quote::quote! {
            #[test]
            fn #test_name() {
    ...
    let err_tests = tests.err.values().sorted_by(|a, b| a.name.cmp(&b.name)).map(|test| {
        let test_name = quote::format_ident!("{}", test.name);
        let test_file = format!("test_data/parser/inline/err/{test_name}.pro");
        quote::quote! {
            #[test]
            fn #test_name() {
...
fn install_tests(tests: &HashMap<String, Test>, tests_dir: PathBuf, check: bool) -> Result<bool> {
    ...
    for (name, test) in tests {
        let path = match existing.get(name) {
            ...
            None => tests_dir.join(name).with_extension("pro"),
        };
...
fn collect_tests(s: &str) -> Vec<Test> {
    ...
    for comment_block in CommentBlock::extract_untagged(s) {
        ...
        let name = match *name.split(' ').collect_vec().as_slice() {
            [name] => name.to_owned(),
...
fn existing_tests(dir: &Path, ok: TestKind) -> Result<HashMap<String, (PathBuf, Test)>> {
    ...
    for file in fs::read_dir(dir)? {
        let path = file?.path();
        let vip_file = path.extension().and_then(|ext| ext.to_str()) == Some("pro");
