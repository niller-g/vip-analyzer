//! Installs vip-analyzer language server and/or editor plugin.
...
fn install_client(sh: &Shell, client_opt: ClientOpt) -> anyhow::Result<()> {
    ...
    let installed_extensions = if cfg!(unix) {
        cmd!(sh, "{code} --install-extension vip-analyzer.vsix --force").run()?;
        cmd!(sh, "{code} --list-extensions").read()?
    } else {
        cmd!(sh, "cmd.exe /c {code}.cmd --install-extension vip-analyzer.vsix --force").run()?;
        cmd!(sh, "cmd.exe /c {code}.cmd --list-extensions").read()?
    };

    if !installed_extensions.contains("vip-analyzer") {
        bail!(
...
fn install_server(sh: &Shell, opts: ServerOpt) -> anyhow::Result<()> {
    ...
    let cmd = cmd!(
        sh,
        "cargo install --path crates/vip-analyzer --profile={profile} --locked --force --features force-always-assert"
    );
    cmd.run()?;
    Ok(())
}
