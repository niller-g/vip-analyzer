...
pub use crate::{
    ...
    syntax_node::{
        ...
        SyntaxToken, SyntaxTreeBuilder, VipLanguage,
    },
...
#[test]
fn smoke_test() {
    let parse = SourceFile::parse("implement a end implement");
    assert_eq!(parse.errors().len(), 0);
    assert_eq!(parse.tree().syntax().text().to_string(), "implement a end implement");
}

#[test]
#[ignore]
fn permute_syntax() {
    const AVG_LENGTH: usize = {
        let mut i = 0;
        let mut sum = 0;
    ...
    fn generator(depth: usize) -> String {
        let mut rng = rand::rng();
        let mut s = String::with_capacity(depth * AVG_LENGTH);
        for _ in 0..depth {
            let kind_index = rand::Rng::random_range(&mut rng, 0..ALL_KINDS.len());
