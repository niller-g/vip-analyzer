...
fn main() -> anyhow::Result<ExitCode> {
    let flags = flags::VipAnalyzer::from_env_or_exit();

    #[cfg(debug_assertions)]
    if flags.wait_dbg || env::var("VA_WAIT_DBG").is_ok() {
        wait_for_debugger();
    }
    ...
    match flags.subcommand {
        flags::VipAnalyzerCmd::LspServer(cmd) => 'lsp_server: {
            if cmd.print_config_schema {
                println!("{:#}", Config::json_schema());
                break 'lsp_server;
            }
            if cmd.version {
                println!("vip-analyzer {}", vip_analyzer::version());
                break 'lsp_server;
            }
...
fn setup_logging(log_file_flag: Option<PathBuf>) -> anyhow::Result<()> {
    ...
    let log_file = env::var("VA_LOG_FILE").ok().map(PathBuf::from).or(log_file_flag);
    ...
    vip_analyzer::tracing::Config {
        writer,
        // Deliberately enable all `error` logs if the user has not set RA_LOG, as there is usually
        // useful information in there for debugging.
        filter: env::var("VA_LOG").ok().unwrap_or_else(|| "error".to_owned()),
        profile_filter: env::var("VA_PROFILE").ok(),
        json_profile_filter: std::env::var("VA_PROFILE_JSON").ok(),
    }
    .init()?;
...
fn run_server() -> anyhow::Result<()> {
    tracing::info!("server version {} will start", vip_analyzer::version());
    ...
    let server_capabilities = vip_analyzer::server_capabilities(&config);

    let initialize_result = lsp_types::InitializeResult {
        capabilities: server_capabilities,
        server_info: Some(lsp_types::ServerInfo {
            name: String::from(env!("CARGO_PKG_NAME")),
            version: Some(vip_analyzer::version().to_string()),
        }),
    ...
    match (vip_analyzer::main_loop(config, connection), io_threads.join()) {
