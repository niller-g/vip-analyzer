...
pub struct Config<T> {
    ...
    /// ```text
    /// env VA_PROFILE=*             // dump everything
    /// env VA_PROFILE=foo|bar|baz   // enabled only selected entries
    /// env VA_PROFILE=*@3>10        // dump everything, up to depth 3, if it takes more than 10
    /// ```
    pub profile_filter: Option<String>,

    /// Filtering syntax, set in a shell:
    /// ```text
    /// env VA_PROFILE_JSON=foo|bar|baz
    /// ```
    ...
    T: for<'writer> MakeWriter<'writer> + Send + Sync + 'static,
{
    pub fn init(self) -> anyhow::Result<()> {
        ...
        let va_fmt_layer = tracing_subscriber::fmt::layer()
            .with_target(false)
            .with_ansi(false)
            .with_writer(writer);

        let va_fmt_layer = match time::OffsetTime::local_rfc_3339() {
            Ok(timer) => {
                // If we can get the time offset, format logs with the timezone.
                va_fmt_layer.with_timer(timer).boxed()
            }
            Err(_) => {
        ...
        let subscriber =
            Registry::default().with(va_fmt_layer).with(json_profiler_layer).with(profiler_layer);
