...
pub(crate) type CheckFixes = Arc<Vec<FxHashMap<FileId, Vec<Fix>>>>;
...
pub(crate) struct DiagnosticCollection {
    ...
    pub(crate) check: Vec<FxHashMap<FileId, Vec<lsp_types::Diagnostic>>>,
...
impl DiagnosticCollection {
    pub(crate) fn clear_check(&mut self, flycheck_id: usize) {
        ...
        self.changes.extend(check.drain().map(|(key, _value)| key));
    ...
    pub(crate) fn clear_check_all(&mut self) {
        ...
        self.changes
            .extend(self.check.iter_mut().flat_map(|it| it.drain().map(|(key, _value)| key)))
    }
    ...
    pub(crate) fn add_check_diagnostic(
        ...
        let diagnostics = self.check[flycheck_id].entry(file_id).or_default();
        ...
        if let Some(fix) = fix {
            ...
            check_fixes[flycheck_id].entry(file_id).or_default().push(*fix);
        }
    ...
    pub(crate) fn diagnostics_for(
        ...
        let check = self.check.iter().filter_map(move |it| it.get(&file_id)).flatten();
        ...
        .filter_map(|file_id| {
            ...
            let diagnostics = match kind {
                NativeDiagnosticsFetchKind::Syntax => {
                    snapshot.analysis.syntax_diagnostics(file_id).ok()?
                }
                NativeDiagnosticsFetchKind::Semantic => snapshot
                    .analysis
                    .semantic_diagnostics(ide_db::assists::AssistResolveStrategy::None, file_id)
                    .ok()?,
            };
...
pub(crate) fn convert_diagnostic(
    ...
    lsp_types::Diagnostic {
        ...
        source: Some("vip-analyzer".to_owned()),
        message: d.message,
