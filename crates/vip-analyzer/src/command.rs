...
/// Vip output is structured one per line. This trait abstracts parsing one line of
/// vip output into a Rust data type.
pub(crate) trait VipCliParser<T>: Send + 'static {
    fn from_line(&self, line: &str, error: &mut String) -> Option<T>;
    fn from_eof(&self) -> Option<T>;
}

struct VipBuilderActor<T> {
    parser: Box<dyn VipCliParser<T>>,
    sender: Sender<T>,
    stdout: ChildStdout,
    stderr: ChildStderr,
}

impl<T: Sized + Send + 'static> VipBuilderActor<T> {
    fn new(
        parser: impl VipCliParser<T>,
        sender: Sender<T>,
        stdout: ChildStdout,
        stderr: ChildStderr,
    ) -> Self {
        let parser = Box::new(parser);
        VipBuilderActor { parser, sender, stdout, stderr }
    }
...
impl<T: Sized + Send + 'static> CommandHandle<T> {
    pub(crate) fn spawn(
        ...
        let actor = VipBuilderActor::<T>::new(parser, sender, stdout, stderr);
        let thread =
    ...
    pub(crate) fn join(mut self) -> io::Result<()> {
        ...
        if read_at_least_one_message || exit_status.success() {
            ...
            Err(io::Error::other(format!(
                "Vip watcher failed, the command produced no valid data (exit code: {exit_status:?}):\n{error}"
            )))
        }
    }
}
