...
import { isVipDocument, isVipprjDocument, sleep, isVipEditor, unwrapUndefinable } from "./util";
...
export function analyzerStatus(ctx: CtxInit): Cmd {
    const tdcp = new (class implements vscode.TextDocumentContentProvider {
        readonly uri = vscode.Uri.parse("vip-analyzer-status://status");
        ...
        async provideTextDocumentContent(_uri: vscode.Uri): Promise<string> {
            ...
            const params: va.AnalyzerStatusParams = {};
            const doc = ctx.activeVipEditor?.document;
            if (doc != null) {
                params.textDocument = client.code2ProtocolConverter.asTextDocumentIdentifier(doc);
            }
            return await client.sendRequest(va.analyzerStatus, params);
        }
    ...
    ctx.pushExtCleanup(
        vscode.workspace.registerTextDocumentContentProvider("vip-analyzer-status", tdcp),
    );
...
export function memoryUsage(ctx: CtxInit): Cmd {
    const tdcp = new (class implements vscode.TextDocumentContentProvider {
        readonly uri = vscode.Uri.parse("vip-analyzer-memory://memory");
        readonly eventEmitter = new vscode.EventEmitter<vscode.Uri>();
        ...
        provideTextDocumentContent(_uri: vscode.Uri): vscode.ProviderResult<string> {
            ...
            return ctx.client.sendRequest(va.memoryUsage).then((mem: any) => {
    ...
    ctx.pushExtCleanup(
        vscode.workspace.registerTextDocumentContentProvider("vip-analyzer-memory", tdcp),
    );
...
export function joinLines(ctx: CtxInit): Cmd {
    return async () => {
        const editor = ctx.activeVipEditor;
        if (!editor) return;

        const client = ctx.client;

        const items: lc.TextEdit[] = await client.sendRequest(va.joinLines, {
...
export function moveItem(ctx: CtxInit, direction: va.Direction): Cmd {
    return async () => {
        const editor = ctx.activeVipEditor;
        if (!editor) return;
        const client = ctx.client;

        const lcEdits = await client.sendRequest(va.moveItem, {
...
export function parentModule(ctx: CtxInit): Cmd {
    return async () => {
        ...
        if (!editor) return;
        if (!(isVipDocument(editor.document) || isVipprjDocument(editor.document))) return;

        const client = ctx.client;

        const locations = await client.sendRequest(va.parentModule, {
            textDocument: client.code2ProtocolConverter.asTextDocumentIdentifier(editor.document),
            position: client.code2ProtocolConverter.asPosition(editor.selection.active),
        });
        if (!locations) return;
    ...
    return async () => {
        ...
        const options: vscode.InputBoxOptions = {
            ...
            validateInput: async (x: string) => {
                try {
                    await client.sendRequest(va.ssr, {
                        query: x,
                        parseOnly: true,
                        textDocument,
                        position,
        ...
        await vscode.window.withProgress(
            ...
            async (_progress, token) => {
                const edit = await client.sendRequest(va.ssr, {
                    query: request,
                    parseOnly: false,
                    textDocument,
                    position,
...
export function serverVersion(ctx: CtxInit): Cmd {
    return async () => {
        if (!ctx.serverPath) {
            void vscode.window.showWarningMessage(`vip-analyzer server is not running`);
            return;
        }
        void vscode.window.showInformationMessage(
            `vip-analyzer version: ${ctx.serverVersion} [${ctx.serverPath}]`,
        );
    };
}
...
export function viewFileText(ctx: CtxInit): Cmd {
    const tdcp = new (class implements vscode.TextDocumentContentProvider {
        readonly uri = vscode.Uri.parse("vip-analyzer-file-text://viewFileText/file.pro");
        readonly eventEmitter = new vscode.EventEmitter<vscode.Uri>();
        constructor() {
        ...
        private onDidChangeTextDocument(event: vscode.TextDocumentChangeEvent) {
            if (isVipDocument(event.document)) {
        ...
        async provideTextDocumentContent(
            ...
            const vipEditor = ctx.activeVipEditor;
            if (!vipEditor) return "";
            const client = ctx.client;
            ...
            return client.sendRequest(va.viewFileText, params, ct);
        }
    ...
    ctx.pushExtCleanup(
        vscode.workspace.registerTextDocumentContentProvider("vip-analyzer-file-text", tdcp),
    );
...
export function reloadWorkspace(ctx: CtxInit): Cmd {
    return async () => ctx.client.sendRequest(va.reloadWorkspace);
}
...
export function applyActionGroup(_ctx: CtxInit): Cmd {
    return async (actions: { label: string; arguments: lc.CodeAction }[]) => {
        ...
        await vscode.commands.executeCommand(
            "vip-analyzer.resolveCodeAction",
...
export function openDocs(ctx: CtxInit): Cmd {
    return async () => {
        ...
        const docLinks = await client.sendRequest(va.openDocs, { position, textDocument });
        log.debug(docLinks);
...
export function cancelFlycheck(ctx: CtxInit): Cmd {
    return async () => {
        await ctx.client.sendNotification(va.cancelFlycheck);
    };
}

export function clearFlycheck(ctx: CtxInit): Cmd {
    return async () => {
        await ctx.client.sendNotification(va.clearFlycheck);
    };
}
...
export function runFlycheck(ctx: CtxInit): Cmd {
    return async () => {
        ...
        await client.sendNotification(va.runFlycheck, { textDocument: params });
    };
}
...
export function viewMemoryLayout(ctx: CtxInit): Cmd {
    return async () => {
        ...
        const position = editor.selection.active;
        const expanded = await client.sendRequest(va.viewRecursiveMemoryLayout, {
            textDocument: client.code2ProtocolConverter.asTextDocumentIdentifier(editor.document),
            position,
        });
...
export function toggleLSPLogs(ctx: Ctx): Cmd {
    return async () => {
        const config = vscode.workspace.getConfiguration("vip-analyzer");
        const targetValue =
