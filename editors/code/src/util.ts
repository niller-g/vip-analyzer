...
class Log {
    private readonly output = vscode.window.createOutputChannel("vip-analyzer Extension", {
        log: true,
    });
...
export type VipDocument = vscode.TextDocument & { languageId: "vip" };
export type VipEditor = vscode.TextEditor & { document: VipDocument };

export function isVipDocument(document: vscode.TextDocument): document is VipDocument {
    ...
    return document.languageId === "vip" && document.uri.scheme === "file";
}

export function isVipprjDocument(document: vscode.TextDocument): document is VipDocument {
    // ideally `document.languageId` should be 'toml' but user maybe not have toml extension installed
    return document.uri.scheme === "file" && document.fileName.endsWith(".vipprj");
}

export function isVipEditor(editor: vscode.TextEditor): editor is VipEditor {
    return isVipDocument(editor.document);
}

export function isDocumentInWorkspace(document: VipDocument): boolean {
